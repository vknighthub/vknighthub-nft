/* eslint-disable @next/next/no-img-element */
import 'tippy.js/dist/tippy.css';

import Tippy from '@tippyjs/react';
import Link from 'next/link';
import React, { useState } from 'react';
import { useDispatch } from 'react-redux';

import { ItemsTabs } from '../../components/component';
import Auctions_dropdown from '../../components/dropdown/Auctions_dropdown';
import Items_Countdown_timer from '../../components/items_countdown_timer';
import Likes from '../../components/likes';
import Meta from '../../components/Meta';
import { items_data } from '../../data/items_data';
import { fetchedAsset, fetchedAssetAddress } from '../../fetchers/asset';
import { bidsModalShow } from '../../redux/counterSlice';
import More_items from './more_items';
import { useQuery } from '@tanstack/react-query';
import stringTruncateFromCenter from './../../util/stringTruncateFromCenter';

const Item = ({ item }) => {
	const dispatch = useDispatch();

	const asset = useQuery(
		["nftasset", item],
		() => fetchedAsset(item),
		{ staleTime: 3000 }
	);

	const assetAddress = useQuery(
		["nftassetaddress", item],
		() => fetchedAssetAddress(item),
		{ staleTime: 3000 }
	);

	if (asset.isError) {
		console.error(asset.error)
	}

	const dataAsset = {}

	if (asset.isSuccess) {
		const data = asset.data
		dataAsset = {
			image: (data && data.last_metadata && data.last_metadata.image.toString().startsWith("ipfs://")) ? data.last_metadata.image.toString().replace("ipfs://", "https://ipfs.io/ipfs/") : '',
			name: data.last_metadata.name,
			projectname: data.project.name,
			policyid: data.policy
		}
	}

	if (assetAddress.isError) {
		console.error(assetAddress.error)
	}
    
	let dataAddress = ''

	if (assetAddress.isSuccess) {
		dataAddress = assetAddress.data[0].address
	}


	const [imageModal, setImageModal] = useState(false);

	return (
		<>
			<Meta title={`vKnightHub | NFT Marketplace`} />
			{/*  <!-- Item --> */}
			<section className="relative lg:mt-24 lg:pt-24 lg:pb-48 mt-24 pt-12 pb-24">
				<picture className="pointer-events-none absolute inset-0 -z-10 dark:hidden">
					<img src="/images/gradient_light.jpg" alt="gradient" className="h-full" />
				</picture>
				<div className="container">

					{dataAsset &&
						<div className="md:flex md:flex-wrap">
							{/* <!-- Image --> */}
							<figure className="mb-8 md:w-2/5 md:flex-shrink-0 md:flex-grow-0 md:basis-auto lg:w-1/2 w-full">
								<button className=" w-full" onClick={() => setImageModal(true)}>
									<img src={dataAsset.image} alt={dataAsset.name} className="rounded-2xl cursor-pointer  w-full" />
								</button>

								{/* <!-- Modal --> */}
								<div className={imageModal ? 'modal fade show block' : 'modal fade'}>
									<div className="modal-dialog !my-0 flex h-full max-w-4xl items-center justify-center">
										<img src={dataAsset.image} alt={dataAsset.name} className="h-full rounded-2xl" />
									</div>

									<button
										type="button"
										className="btn-close absolute top-6 right-6"
										onClick={() => setImageModal(false)}
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											viewBox="0 0 24 24"
											width="24"
											height="24"
											className="h-6 w-6 fill-white"
										>
											<path fill="none" d="M0 0h24v24H0z" />
											<path d="M12 10.586l4.95-4.95 1.414 1.414-4.95 4.95 4.95 4.95-1.414 1.414-4.95-4.95-4.95 4.95-1.414-1.414 4.95-4.95-4.95-4.95L7.05 5.636z" />
										</svg>
									</button>
								</div>
								{/* <!-- end modal --> */}
							</figure>

							{/* <!-- Details --> */}
							<div className="md:w-3/5 md:basis-auto md:pl-8 lg:w-1/2 lg:pl-[3.75rem]">
								{/* <!-- Collection / Likes / Actions --> */}
								<div className="mb-3 flex">
									{/* <!-- Collection --> */}
									<div className="flex items-center">
										<Link href="#">
											<a className="text-accent mr-2 text-sm font-bold">{dataAsset.projectname}</a>
										</Link>
										<span
											className="dark:border-jacarta-600 bg-green inline-flex h-6 w-6 items-center justify-center rounded-full border-2 border-white"
											data-tippy-content="Verified Collection"
										>
											<Tippy content={<span>Verified Collection</span>}>
												<svg className="icon h-[.875rem] w-[.875rem] fill-white">
													<use xlinkHref="/icons.svg#icon-right-sign"></use>
												</svg>
											</Tippy>
										</span>
									</div>

									{/* <!-- Likes / Actions --> */}
									<div className="ml-auto flex items-stretch space-x-2 relative">
										<Likes
											like={1}
											classes="dark:bg-jacarta-700 dark:border-jacarta-600 border-jacarta-100 flex items-center space-x-1 rounded-xl border bg-white py-2 px-4"
										/>
									</div>
								</div>

								<h1 className="font-display text-jacarta-700 mb-4 text-4xl font-semibold dark:text-white">
									{dataAsset.name}
								</h1>



								{/* <!-- Creator / Owner --> */}
								<div className="mb-8 flex flex-wrap">
									<div className="mr-8 mb-4 flex">
										<figure className="mr-4 shrink-0">
											<Link href="/user/avatar_6">
												<a className="relative block">
													<img
														src={dataAsset.image}
														alt={dataAsset.image}
														className="rounded-2lg h-12 w-12"
														loading="lazy"
													/>
													<div
														className="dark:border-jacarta-600 absolute -right-3 top-[60%] flex items-center justify-center "
														data-tippy-content="Verified Collection"
													>
														<Tippy content={<span>This project is receiving 5.00% royalties which goes back to the creator with each sale!</span>}>
															<svg className="icon h-[30px] w-[30px]">
																<use xlinkHref="/icons.svg#icon-royalty"></use>
															</svg>
														</Tippy>
													</div>
												</a>
											</Link>
										</figure>
										<div className="flex flex-col justify-center ">
											<span className="text-jacarta-400 block text-sm dark:text-orange-bright">
												<strong>7.00% royalties</strong>
											</span>
										</div>
									</div>

									<div className="mb-4 flex">
										<figure className="mr-4 shrink-0">
											<Link href="/user/avatar_6">
												<a className="relative block">
													<img
														src={dataAsset.image}
														alt={dataAsset.image}
														className="rounded-2lg h-12 w-12"
														loading="lazy"
													/>
													<div
														className="dark:border-jacarta-600 bg-green absolute -right-3 top-[60%] flex h-6 w-6 items-center justify-center rounded-full border-2 border-white"
														data-tippy-content="Verified Collection"
													>
														<Tippy content={<span>Verified Collection</span>}>
															<svg className="icon h-[.875rem] w-[.875rem] fill-white">
																<use xlinkHref="/icons.svg#icon-right-sign"></use>
															</svg>
														</Tippy>
													</div>
												</a>
											</Link>
										</figure>
										<div className="flex flex-col justify-center ">
											<span className="text-jacarta-400 block text-sm dark:text-white">
												Owned by 
											</span>
											<Link href="/user/avatar_6">
												<a className="text-accent block">
													<span className="text-sm font-bold" >{stringTruncateFromCenter(dataAddress,15)}</span>
												</a>
											</Link>
										</div>
									</div>
								</div>

								{/* <!-- Bid --> */}
								<div className="dark:bg-jacarta-700 dark:border-jacarta-600 border-jacarta-100 rounded-2lg border bg-white p-8">
									<div className="mb-8 sm:flex sm:flex-wrap">
										{/* <!-- Highest bid --> */}
										<div className="sm:w-1/2 sm:pr-4 lg:pr-8">
											<div className="block overflow-hidden text-ellipsis whitespace-nowrap">
												<span className="dark:text-jacarta-300 text-jacarta-400 text-sm">
													Policy ID: {' '}
												</span>
												<Link href="/user/avatar_6">
													<a className="text-accent text-sm font-bold">
														{stringTruncateFromCenter(dataAsset.policyid,15)}
													</a>
												</Link>
											</div>
											
										</div>

										{/* <!-- Countdown --> */}
										<div className="dark:border-jacarta-600 sm:border-jacarta-100 mt-4 sm:mt-0 sm:w-1/2 sm:border-l sm:pl-4 lg:pl-8">
											<span className="js-countdown-ends-label text-jacarta-400 dark:text-jacarta-300 text-sm">
												Asset ID: {' '}
											</span>
											<Link href="/user/avatar_6">
													<a className="text-accent text-sm font-bold">
														{stringTruncateFromCenter(item,15)}
													</a>
												</Link>
										</div>
									</div>

									<Link href="#">
										<button
											className="bg-accent shadow-accent-volume hover:bg-accent-dark inline-block w-full rounded-full py-3 px-8 text-center font-semibold text-white transition-all"
											onClick={() => dispatch(bidsModalShow())}
										>
											Place Bid
										</button>
									</Link>
								</div>
								{/* <!-- end bid --> */}
							</div>
							{/* <!-- end details --> */}
						</div>
					}
					<ItemsTabs />
				</div>
			</section>
			{/* <!-- end item --> */}

			<More_items />
		</>
	);
};

export default Item;

Item.getInitialProps = ({ query: { item } }) => {
	return { item }
}
